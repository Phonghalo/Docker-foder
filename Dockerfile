...
RUN apt-add-repository ppa:ondrej/php && apt-get update\
    &&  apt-get -y install --fix-missing php8.0 php8.0-fpm php8.0-mysqli php8.0-curl \
...
# PHP7.2 && Extensions
RUN apt-add-repository ppa:ondrej/php && apt-get update\
    && apt-get -y install --fix-missing php7.2 php7.2-fpm php7.2-mysqli php7.2-json php7.2-curl php7.2-dev\
    ...
    php7.2-mbstring php7.2-gd php7.2-bcmath php7.2-mbstring

RUN apt-add-repository ppa:ondrej/php && apt-get update\
    && apt-get -y install --fix-missing php7.4 php7.4-fpm php7.4-mysqli php7.4-json php7.4-curl php7.4-dev\
...
CMD service nginx start && service php7.4-fpm start && service php7.2-fpm start && service redis-server start && tail -f /dev/null
